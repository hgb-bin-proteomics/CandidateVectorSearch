...
data <- read_xlsx("benchmark_pc_2500000.xlsx")
...
grp09_palette = c("#2B2D42", "#383B50", "#44485D", "#5C6378",
                  "#BDC6D1", "#D5DCE3", "#E1E7EC", "#EDF2F4",
                  "#EF233C")

text_col <- c(rep("white", 4), rep("black", 4), rep("white", 1))
...
ggplot(data, aes(x = Method, y = Mean, fill = Method)) +
  ...
  scale_fill_manual(values = grp09_palette) +
  ggtitle("Execution time of the different methods using 2 500 000 simulated candidates.") +
  xlab("Method") +
  ylab("Time (s)") +
